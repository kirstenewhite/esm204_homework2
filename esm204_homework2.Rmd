...
- Each district has to reduce `r round(a_stringency, 3)`% from their starting baseline to reach the abatement target of 500 AF.  
  - Kern reduces `r round(kern_use_reduction, 2)` units from `r kern_baseline` down to `r round(kern_abatement_cap, 2)`
  - Mojave reduces `r round(mojave_use_reduction, 2)` units
  - Antelope reduces `r round(antelope_use_reduction, 2)` units
  - Ventura reduces `r round(ventura_use_reduction, 2)` units
