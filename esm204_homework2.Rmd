...
Under this policy, the stringency will be the same as Policy A - 0.338% and there will be no tax revenue generated

The cost to each district and the total program cost under a cap without trade policy was found in Policy A (above). 

The equimarginal principle tells us that units will be bought and sold at the same tax rate as calculated in Policy B ($158). Therefore, the total cost of meeting the 500 AF abatement target using a Cap and Trade policy will be the same as the cost under the Tax policy,  $21,1763.16. 
...
kern_300cost_abatement <- (0.5 * kern_300_use_reduction * kern_300_price_abatement)
# y (P* ) = 343.5 - 2.29 (99.3) =  115.20; 
# therefore the total cost to abate is the area under the curve = (1/2 * 50.7 * 115.20) = $2,938.83
# kern total cost = $2,938.83
...
(1) Each district has to reduce `r round(four_stringency, 3)*100`% from their starting baseline to reach the abatement target of 300 AF.  
  ...
  - Kern reduces `r round(kern_300_use_reduction, 2)` units from `r kern_abatement_cap` AF down to `r round(kern_300_abatement_cap, 2)` AF at a price of `r round(kern_300price_abatement, 2)` $/AF. 
  - Mojave reduces `r round(mojave_300_use_reduction, 2)` units from `r mojave_abatement_cap` AF down to `r round(mojave_300_abatement_cap, 2)`AFat a price of `r round(mojave_300_price_abatement, 2)` $/AF.
  - Antelope reduces `r round(antelope_300_use_reduction, 2)` units from `r antelope_abatement_cap` AF down to `r round(antelope_300_abatement_cap, 2)` AF at a price of `r round(antelope_300_price_abatement, 2)` $/AF.
  - Ventura reduces `r round(ventura_300_use_reduction, 2)` units from `r ventura_abatement_cap` AF down to `r round(ventura_300_abatement_cap, 2)` AF at a price of `r round(ventura_300_price_abatement, 2)` $/AF.
